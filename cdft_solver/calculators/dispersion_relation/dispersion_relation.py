...
def dispersion_relation(
    ...
    def compute_dispersion_relation(
        ...
        """
        Compute dispersion relation ω(k) for multicomponent DDFT.
        ...
        c_r : (N,N,Nr)
        densities : (N,)
        D_matrix : (N,N)  (can be diagonal or full)
        k_values : array (optional)

        Returns
        -------
        k_vals, omega_k (Nk, N eigenvalues)
        """

        N = len(densities)

        # --- transform c(r) → c(k)
        c_k, k_grid = hankel_transform_matrix_fast(c_r, r)
        ...
        for idx, k in enumerate(k_values):
            ...
            # --- build thermodynamic matrix
            Ck = np.zeros((N, N))
            for i in range(N):
                for j in range(N):
                    Ck[i, j] = (1.0 if i == j else 0.0) - densities[j] * c_k[i, j, idx]

            # --- kinetic matrix (k^2 * D)
            K = (k**2) * D_matrix
            ...
            # dispersion relation: ω = -λ
            omega_k[idx, :] = -np.real(eigvals)

        return k_values, omega_k
        
    # ============================================================
    # STEP: Dispersion relation (DDFT)
    # ============================================================

    if diffusivity is None:
        raise ValueError("❌ diffusivity_matrix must be provided")

    k_vals, omega_k = compute_dispersion_relation(
        r=r,
        c_r=c_r,
        densities=np.asarray(densities, float),
        D_matrix=np.diag(np.asarray(diffusivity, float)),
    )
    ...
    if export:
        ...
        json_out["dispersion_relation"] = dispersion_data
