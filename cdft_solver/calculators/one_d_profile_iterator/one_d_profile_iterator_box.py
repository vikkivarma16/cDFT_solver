...
def one_d_profile_iterator_box(ctx, config, export_json= True, export_plots = True, filename  = "one_d_profiles"):   
    ...
    alpha = profile_p [ "alpha_mixing_min" ]
    ...
    _, pressure, _ = func_pressure(rho_point, vij_bulk)
    ...
    def mf_convolution_kernel(
        ...
        for z1 in prange(nx):
            ...
            for z2 in range(nx):
                vij = vij_kj[z1, z2]
                conv_A += vij * del_rhoA_2[z2]
                conv_B += vij * del_rhoB_2[z2]
                if (i==0):
                    conv_L += vij * rho_2_factor[z2]
    ...
    while (iteration < iteration_max):
        ...
        for i in range(nx):        
            ...
            for particle in species:
                ...
                exponent = -v_ext[particle][i] / temperature + mue_r[pid][i] - free_energy
...
# -------------------------
        ...
        if residual < prev_residual:
            ...
            alpha = max(alpha * 0.98, alpha_min)
        ...
        if (iteration + 1) % log_period == 0:
            ...
            pressure_line = f"Pressure value is given as: {pressure_values[int(3*nx/4)]:.6e}\n\n"
